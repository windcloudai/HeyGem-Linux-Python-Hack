...
from utilmeta_config import TTS_URL, TTS_OUTPUT, TTS_TOKEN
...
try:
    ...
    from h_utils.custom import (
        CustomError as ActualCustomError,
    )  # 假设h_utils也在HeyGem-Linux-Python-Hack下
    # 初始化配置，这非常重要，路径需要正确
    ...
    print(
        f"And that necessary __init__.py files exist in subdirectories like 'service' and 'y_utils'."
    )
    ...
    print(
        "WARNING: Failed to import actual modules, API might not function correctly or use mocks."
    )
...
def write_video_adapted(
    ...
    try:
        ...
        logger.error(
            f"Custom VideoWriter [{work_id}] timed out waiting for image queue."
        )
        if video_write is not None:
            video_write.release()
        return None  # 或者抛出异常
...
class MockTransDhTask:
    ...
    def work(
        ...
        logger.info(
            f"MockTransDhTask.work called with audio: {audio_path}, video: {video_path}"
        )
        ...
        mock_frame = cv2.imread(
            "/home/ai/projects/fengyun-heygem/HeyGem-Linux-Python-Hack/1.jpeg"
        )  # 使用项目中的一个图片作为模拟帧
        ...
        for _ in range(num_mock_frames):
            self.output_imgs_queue.put(
                (None, None, [mock_frame.get()])
            )  # 放入实际的 numpy array
...
class VideoRequestSchema(Schema):
    anchor_model: str
    anchor_tts_model: str
    text: str
    configure: ConfigureSchema = None
    

def request_tts(model, text, volume, rate, pitch):
    ...
    if result.code != AS.OK:
        return RS.error(AS.INTERNAL_SERVER_ERROR, "TTS service request failed")
    # Parse response JSON
    tts_data = result.data
    audio_file = tts_data["audio"]
    audio_url = TTS_OUTPUT + "/" + audio_file
    
    # Download audio file from TTS service
    audio_resp = requests.get(audio_url)
    if audio_resp.status_code != http.HTTPStatus.OK:
        return RS.error(AS.INTERNAL_SERVER_ERROR, "Failed to download audio file from TTS service")
...
class VideoGenerationAPI(api.API):
    ...
    @jwt_required
    async def generate_video(self, req: VideoRequestSchema = request.Body):
        if not actual_trans_dh_service or not GlobalConfig or not actual_logger:
            return response.Response(
                "Core video processing modules are not available.", status=500
            )
        ...
        video_path = os.path.join(
            ...
            "assets",
            req.anchor_model,
            f"{req.configure.figure_type}_{req.configure.clothes_type}.mp4",
        )
        ...
        if not os.path.exists(video_path):
            video_path = os.path.join(
                ...
                req.anchor_model,
                "default.mp4")
            return RS.error(AS.DATA_NOT_FOUND, "视频模板不存在")
        
        audio_path = request_tts(
            req.anchor_tts_model,
            req.text,
            req.configure.volume or "",
            req.configure.rate or "", 
            req.configure.pitch or ""
        )
        ...
        logging.info(
            f"Does path exist (from api.py perspective after chdir)? {path_exists}"
        )
        # +++ 结束新增调试代码 +++
        ...
        try:
            ...
            if hasattr(actual_trans_dh_service, "TransDhTask"):
                logging.info(
                    "TransDhTask init (attempting instantiation)"
                )  # 修改日志以区分
                task_instance = actual_trans_dh_service.TransDhTask()
            else:
                actual_logger.error(
                    "TransDhTask class not found in imported service.trans_dh_service"
                )
                return RS.error(
                    AS.INTERNAL_SERVER_ERROR, "Video processing task handler not found."
                )
            ...
            if output_video_path and os.path.exists(output_video_path):
                ...
                actual_logger.error(
                    "Video generation failed or output file not found after task.work."
                )
                return RS.error(
                    AS.INTERNAL_SERVER_ERROR,
                    "Video generation failed or output file not found.",
                )

        except ActualCustomError as e:
            actual_logger.error(f"CustomError during video generation: {e}")
            return RS.error(
                AS.INTERNAL_SERVER_ERROR, f"Video generation error: {str(e)}"
            )
        except Exception as e:
            actual_logger.error(
                f"Unexpected error during video generation: {e} - {traceback.format_exc()}"
            )
            return RS.error(
                AS.INTERNAL_SERVER_ERROR,
                f"Video generation failed due to an unexpected error: {str(e)}",
            )
        finally:
            # --- Begin: Revert CWD ---
